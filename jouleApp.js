...
id('addElement').addEventListener('click',function() {
	...
	if(element.type!='opening') { // except openings first layer  is always...
		layer=pseudoLayers[0]; // ...inner surface
		switch(element.type) {
			case 'wall':
				layer.r=-0.15;
				break;
...
function seedMaterials() {
	materials=[{'name':'concrete','r':0.69,'v':200,'t':-1},
...
function listProjects() {
	var listItem;
	id('headerTitle').innerText='Joule projects';
...
function listLayers() {
	...
	for(var i in layers) {
		...
		listItem=document.createElement('li');
		listItem.index=i;
		listItem.innerText=layer.name;
		if(layer.r<0) r-=layer.r; // negative r represents resistance...
		else r+=layer.thickness*layer.r; // ...otherwise resistivity
		// add click action
		id('list').appendChild(listItem);
	}
